...
class RateTask:
    ...
    async def execute(self) -> str:
        ...
        try:
            async with async_session_maker() as session:
                ...
                # 批量获取所有待评价订单，按account_id分组（避免N+1查询）
                account_ids = [a.account_id for a in accounts]
                orders_by_account = await self._get_pending_rate_orders_batch(session, account_ids)
    ...
    async def _get_pending_rate_orders_batch(
        self,
        session: AsyncSession,
        account_ids: List[str]
    ) -> Dict[str, List[XYOrder]]:
        """
        批量获取多个账号的待评价订单（避免N+1查询）
    ...
    async def _process_account(
        ...
        # 获取待评价订单（优先使用批量预取的数据）
        orders = pre_fetched_orders if pre_fetched_orders is not None else await self._get_pending_rate_orders(session, account_id)

...
| P6 | Bug 修复（AI 参数缺失 / 依赖遗漏 / MySQL 专有函数） |
| P7 | 安全加固（硬编码密钥→DB 配置 / HTTP→HTTPS / 域名常量化） |
| P8 | 数据库优化（5 个复合索引 / N+1→批量查询 / 关键词+设置缓存） |
| P9 | 模块集成（Redis 冷却 / Future 管理器 / 发货管道可选开关） |
| P10 | 日志规范（20+ 处静默异常添加日志 / 前缀统一 ModuleName:） |
